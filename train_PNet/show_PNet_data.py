...
def show_pnet_data(data_path, num_samples=10):
    """
    显示PNet训练数据，随机选择每种类型的样本并拼接成一个大图
    参数：
      data_path: 数据路径
    ...
    min_cell_size = 60
    cell_width = max(img_width * 4, min_cell_size)  # PNet图像很小，放大4倍
    cell_height = max(img_height * 4, min_cell_size)
    ...
    # 显示图像
    cv2.imshow('PNet Training Data', collage)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    
    # 保存图像
    output_path = os.path.join(os.path.dirname(data_path), 'PNet_data_preview.png')
...
if __name__ == '__main__':
    ...
    show_pnet_data(data_path, num_samples=10)
