...
def show_onet_data(data_path, num_samples=10):
    """
    显示ONet训练数据，随机选择每种类型的样本并拼接成一个大图
    参数：
      data_path: 数据路径
    ...
    # 根据图像尺寸设置单元格大小
    cell_width = img_width * 2  # ONet图像48x48，放大2倍
    ...
    # 显示图像
    cv2.imshow('ONet Training Data', collage)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    
    # 保存图像
    output_path = os.path.join(os.path.dirname(data_path), 'ONet_data_preview.png')
...
if __name__ == '__main__':
    ...
    show_onet_data(data_path, num_samples=10)
